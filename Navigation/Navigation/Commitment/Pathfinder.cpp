...
array<array<double> > Pathfinder::build_array(double width, double height){
	//create a two dimensional array storing the coordinates on the field
	array<array<double> > result(width, array<double> height);
	...
	pair<double, double> meterCoodinates;

	for(int j = 0; j < height; j++){
		for(int i = 0; i < width; i++){
			meterCoodinates = arrayToMeters(i,j);
			double x = meterCoodinates.first;
			double y = meterCoodinates.second;
			if (isInField(x,y)){
				//cout<< "i: " <<i<<"j: "<<j<<endl;
				result[i][j] = 1;
			else{
...
*/
Bitmap Pathfinder::create_image(array<Node^ >^ pathArr, array<double>^ arr, String^ filename){
	Bitmap image(filename);

	image = Bitmap(arr->Length,arr[0]->Length);
	...
	int count = 0
		...
		for (int i= 0; i< pathArr->Length; i++){
			...
			//coordinates
			double start_x = prev_node->x;
			double start_y = prev_node->y;
			double stop_x = node->x;
			double stop_y = node->y;

			Graphics ^ graphics = panel->CreateGraphics(); 
			Point start(start_x,start_y); 
			Point stop(stop_x, stop_y); 

			for(int i= 0; i< prev_node->children ->Length; i++){
				Node^ child = prev_node ->children[i];
				Point child(child->x, child->y);
				if ( child !=0 &&  (prev_node -> j )% 2 == 1){
					Pen^ penCurrent = gcnew Pen('#00FF00'); 
					graphics->DrawLine(penCurrent,start,child); 
				}
			}
			Pen^ penCurrent = gcnew Pen('#000000');
			graphics->DrawLine(penCurrent,start,stop);
			//cli c++ does not support draw text....
			graphics->DrawText(count.ToString(), start);
			prev_node = node;
			count += 1;
		}

		graphics->DrawText(count.ToString(), stop);

		//how to convert graphics to a bitmap
		image.Save(filename)
			cout<< "Done!"<<endl;
}
...
//return coordinates(2D)
//how does array associate with file? IO?
array<double> create_file(Autosearch^ autosearch,String^ filename){
	f = open(filename,'w');
	for(int i = 0; i< path->Length; i++){
		//path is an array of coordinates, why node in path????
		Node^ node = path[i];
...
Point get_closest_valid_point(Point^ point, array<double> mapArr, array<double> perimeters, Point^ center,int index){
...
array<double> dfs_recursive(node,direction){
	...
	array<double> dfs(self,node,direction){
		...
		try{
			while(1){
				...
				try{
					// confusion whattt?
					child,direction = pick_best(node,direction);
					stack.append(child);
					node = child;
				}
				catch(PathFinishedException& e){
	...
	Node^ pick_best(self,node,direction){
